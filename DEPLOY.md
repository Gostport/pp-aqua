...
На странице «Правила и данные» обещано, что журнала обращений нет и
IP-адреса посетителей никуда не пишутся. Сам аквариум их и правда не
пишет: в его лог попадает только «в такой-то аквариум добавлена рыбка».
Но запросы идут через прокси, и обещание держится ровно настолько,
насколько это верно для него.

Проверь у Traefik `accessLog` — в статическом конфиге (`traefik.yml`
или флаги контейнера). Если он включён, IP-адреса всех, кто смотрит
детские рисунки, ложатся на диск, и текст правил надо либо поправить,
либо выключить лог для этого роутера.
