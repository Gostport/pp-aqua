...
- **Пароль ходит открытым текстом** в заголовке `X-Tank-Pass`. В домашней
  сети это приемлемо, в интернете — нет: нужен HTTPS (обратный прокси).
  На сервере от пароля хранится только scrypt-хеш с солью.
- **Добавление рыбок и загрузка фонов без пароля** — осознанное решение
  для домашней сети. В открытом интернете любой со ссылкой набьёт диск
  картинками: нужен либо лимит, либо закрытый доступ (VPN, Tailscale,
  basic-auth на прокси).
