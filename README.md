...
«🐠 Запустить готовую рыбку» — должно показать 28 карточек с миниатюрами.
Если карточек нет, посмотри `http://localhost:8000/api/pack`: пусто —
значит пак не собрался, ищи в выводе скрипта строки «пропуск».
